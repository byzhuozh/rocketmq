...
public class MappedFile extends ReferenceResource {
    ...
    private void init(final String fileName, final int fileSize) throws IOException {
        ...
        try {
            ...
            // 此时建立了进程虚拟地址空间，并没有分配虚拟内存对应的物理内存， 只有写入文件的时候，虚拟地址和物理内存地址才真正建立关系(通过页表建立联系)
    ...
    /**
     * @param flushLeastPages flush 最小页数
     *
     * @return The current flushed position
     */
    public int flush(final int flushLeastPages) {
       //校验写入的位置和刷盘的位置，是不是超过4k, 【内存页是 4k】
    ...
    public int commit(final int commitLeastPages) {
        ...
        // All dirty data has been committed to FileChannel.  写到文件尾时，回收 writeBuffer。
        // 将 writeBuffer缓冲区 归还到 内存池
    ...
    public void warmMappedFile(FlushDiskType type, int pages) {
        ...
        for (int i = 0, j = 0; i < this.fileSize; i += MappedFile.OS_PAGE_SIZE, j++) {
            // 每一个 4k 页，写入一个字节 0， 【linux默认分页就是4K】
        ...
        //锁定内存，防止OS因为内存不够了，把这块内存交换到swap去
